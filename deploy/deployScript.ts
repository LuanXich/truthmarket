...
function MarketDetail({
  ...
  const handleResolve = async () => {
    ...
    try {
      ...
      setMsg({ type: "ok", text: "Resolution submitted! Validators are fetching web data + running LLM jury..." });
      onRefresh();
    } catch (e: unknown) {
      ...
      <div className="bg-dark-800 border border-dark-600 rounded-2xl p-6 mb-4">
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-2">
              ...
              <button
                ...
                className={`py-3 rounded-lg font-semibold text-sm transition-all ${
                  !betSide ? "bg-red-600 text-white shadow-[0_0_20px_rgba(239,68,68,0.3)]" : "bg-dark-700 text-gray-400 hover:text-white"
                }`}
              >
                👎 NO
              </button>
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={handleBet}
                disabled={loading}
                className="px-5 py-2.5 bg-brand-500 hover:bg-brand-600 disabled:opacity-50 text-white rounded-lg font-semibold text-sm transition-colors glow-green"
              >
                {loading ? "..." : "Place Bet"}
              </button>
            </div>
        ...
        {market.status === "resolved" && (
          <button
            onClick={handleClaim}
            disabled={loading}
            className="w-full py-3 bg-brand-500 hover:bg-brand-600 disabled:opacity-50 text-white rounded-lg font-semibold transition-colors glow-green flex items-center justify-center gap-2"
          >
